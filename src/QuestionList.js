...
class QuestionList extends Component {
    ...
    render() {
        ...
        const questions = this.state.data && this.state.data.filter(v => v.text.toLowerCase().indexOf(this.props.searchTerm) >= 0);
        return (<section id="questions">

            <div className="container">
                {questions && <div className="question-count">Shown {questions.length} out of {this.props.questionsData.length}</div>}
                {questions &&
                questions.map((v, i) => <QuestionBox onTrigger={onTrigger} key={i} qidx={i} question={v}/>)}
                <div className="container-button">
                    <div>
                        <button onClick={() => this.removeCorrect()} className="button correct">Remove Correctly
                            Answered Questions
                        </button>
                    </div>
                    <div>
                        <button onClick={() => this.resetAllQuestions()} className="button reset">Reset all questions
                        </button>
                    </div>
                </div>
            </div>
